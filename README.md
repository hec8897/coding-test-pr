...
| 조합                   | Lv.0            | ⬜   | [📁](./programmers/lv0_조합)                   | 백트래킹, 재귀, 조합 ⭐⭐⭐            |
...
|                        | 조이스틱        | Lv.2 | ✅                                             | [📁](./programmers/lv2_조이스틱)        | 그리디, 최적화, 문자열 ⭐⭐⭐ |
...
- 실전 코테 문제 정리: 모노토닉 큐/덱 + 슬라이딩 윈도우 3문제
- 길이 ≤ k 최대 부분합 (누적합 + 모노토닉 덱)
- 최대 k 점프로 도달하는 최소 비용 (DP + 슬라이딩 윈도우)
- 일관된 로그 최대 길이 (빈도의 빈도 관리)
- 코테 30분전 체크리스트 문서 추가

#### 📁 2025-11-10 (월)

- 코테 대비 Lv.2 핵심 복습 세션 (DFS/BFS + 구현)
- 기능개발 재복습: 남은 일수 계산, forEach 분기 검증
- 타겟 넘버 BFS 변형 시도: 큐 상태 관리 아이디어 점검
- 주식가격 복습: O(n^2) 기본 로직 확인 및 스택 풀이 비교
- 조이스틱 복습: 상하 조작 + 좌우 최소 이동 전략 재정리
- 124 나라의 숫자 복습: 나머지 0 처리와 문자열 누적 방식 정리

#### 📁 2025-11-04 (화)

- 그리디 알고리즘 집중 복습 세션
- 구명보트 복습: 투 포인터 + 그리디 패턴 완벽 이해
- 체육복 복습3: 그리디 + 순회 + 배열 상태 관리 마스터
- 큰 수 만들기 복습: 그리디 + 스택, while 조건 3가지 핵심 이해
- 조이스틱 문제 추가: 그리디 최적화 (고급), 연속 A 구간 처리
- charCodeAt() 메서드 학습 및 활용법 이해

#### 📁 [2025-11-03 (월)](./daily-logs/2025-11-03.md)

- 1차 모의고사 (복습 세션): 3문제 완료
- 전화번호 목록 복습: Hash/Set 부분문자열 패턴 재확인
- 짝지어 제거하기 복습: Stack으로 연속 문자 제거
- 올바른 괄호 복습: Stack 짝맞추기 패턴 재확인
- 코테 전날 워밍업 및 패턴 복습

#### 📁 [2025-10-31 (금)](./daily-logs/2025-10-31.md)

- Day 4 완료! 그리디 + 구현 + 재귀 3가지 패턴 마스터
- 복습 세션: 4문제 (폰켓몬, 같은 숫자는 싫어, K번째수, 타겟 넘버)
...
- 팩토리얼 재귀 풀이: 단순 재귀 패턴 연습
- 124 나라의 숫자: 진법 변환, while 루프, 패턴 찾기
- 타겟 넘버 개념 이해: 분기 재귀, 개수의 합, 트리 구조
- 조합 구현 완료: 백트래킹, push/pop, 상태 복구
